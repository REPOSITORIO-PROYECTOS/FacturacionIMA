...
function buildTargets(username: string, isDeactivate: boolean): string[] {
  const suffix = isDeactivate ? `usuarios/${username}/desactivar` : `usuarios/${username}`;
  return basesUsr.flatMap((baseUrl) => {
    const base = baseUrl.replace(/\/+$/, "");
    return [`${base}/${suffix}`, `${base}/api/${suffix}`];
  });
}

async function forward(username: string, request: Request, options: { isDeactivate: boolean; method: "PUT" | "POST" }): Promise<Response> {
  ...
  const targets = buildTargets(username, options.isDeactivate);
  ...
  return new Response(JSON.stringify({ detail: "Error de conexión usuarios (dynamic)" }), {
    status: 500,
    headers: { "Content-Type": "application/json" },
  });
}
...
export async function PUT(request: Request, context: any): Promise<Response> {
  ...
  return forward(username, request, { isDeactivate: false, method: "PUT" });
}
